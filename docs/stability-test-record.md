...
- 自定义障碍物功能已完成 batchmode 场景生成与主场景烟雾回归，但尚未纳入打包版长循环稳定性测试。
- 新增算法过程可视化功能已完成源码接入与编辑器编译回归，但尚未纳入打包版长循环稳定性测试。

## 后续人工复核
- 使用运行时面板手动验证 `JSON` 导出与批量实验，补充 `session_manifest.json`、`session_summary.csv` 实际产物。
- 构造一组明确穿越建筑 footprint 的任务点，用于专项验证建筑告警确实会触发。
- 在 `CustomObstacleSandbox` 中执行“绘制 -> 删除 -> 清空 -> 开始仿真 -> 重置”循环，补测障碍缓存刷新与路径规划稳定性。
- 在 `CustomObstacleSandbox` 中补测“样式切换 -> 缩放 -> 创建楼体 -> 开始仿真”流程，确认城市楼体模板和缩放倍率在长循环下不会破坏障碍缓存。
- 人工确认鼠标停留在运行时控制面板、Scroll View 和输入框上滚动时，场景相机不会响应滚轮缩放。
- 在打包版中执行“播放 -> 暂停 -> 单步 -> 重置 -> 切换无人机 -> 切换模式 -> 切换倍速”循环，补测算法过程演示链路的状态一致性。
