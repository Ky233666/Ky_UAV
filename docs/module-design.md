...
| 障碍物编辑模块 | 运行时拖拽创建、删除、清空自定义建筑并刷新障碍缓存 | 鼠标拖拽、地面点击、高度参数 | `RuntimeObstacleMarker` 集合、更新后的障碍层与 footprint | 依赖 `RuntimeObstacleEditor`、`SimulationRuntimeControlPanel`、`DroneManager` | 已完成（箱体建筑第一版） |
...
| 交互界面模块 | 提供场景按钮和右侧运行时控制面板 | 用户点击、快捷键 | 参数更新、控制命令、状态显示 | 依赖 `TaskPointUIManager`、`SimulationRuntimeControlPanel`、`RuntimeObstacleEditor` | 已完成 |
...
- `RuntimeObstacleEditor` 当前只支持轴对齐箱体建筑，且障碍物布局为运行时会话级对象，不会自动持久化到场景文件。
