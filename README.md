...
- 支持统一的路径规划算法过程可视化，可按无人机回放 `StraightLine`、`AStar`、`RRT` 的规划步骤，区分已访问节点、当前扩展节点、候选路径、回溯路径和最终路径。
- 支持无人机生成、重建、任务分配、状态推进、重置和简单局部避让。
- 支持任务点运行时新增、清空、导入。
- 支持起飞点运行时新增、移动、删除、清空。
- 支持在运行时通过右侧面板拖拽创建、删除、清空自定义障碍物，可在长方体与城市楼体模板间切换，并调节缩放倍率与高度，自动刷新规划障碍层和建筑 footprint。
- 支持统一 UI 输入拦截；鼠标悬停在运行时面板、滚动区、输入框和下拉控件上时，场景相机滚轮和视角控制不会误响应。
...
- 结果对比仍主要依赖导出文件和批量实验；当前虽已支持路径规划过程回放，但还没有独立的结果对比仪表板或双算法并排回放界面。
...
- `Assets/Scripts/UAV/Controller`
  - 仿真控制、无人机管理、状态机、相机、运行时面板、算法过程可视化、导出、任务点与起飞点交互。
- `Assets/Scripts/UAV/Comm`
  - 调度算法、路径规划算法、算法接口、过程记录接口、可视化轨迹构建器、算法名称映射、实验预设目录构建。
- `Assets/Scripts/UAV/Config`
  - `DroneConfig` 默认参数配置。
- `Assets/Scripts/UAV/Model`
  - 调度请求/结果、路径请求/结果、路径规划可视化模型、实验导出模型、实验预设模型。
- `Assets/Editor`
  - 交付资产初始化、烟雾验证、批量测试、Windows 打包、城市场景导入、环境修复与 sandbox 场景生成工具。
- `Assets/Tests/EditMode/Editor`
  - 调度、规划、导出、算法名称映射、实验预设目录逻辑测试，以及路径规划过程可视化逻辑测试。
...
6. 使用右侧运行时控制面板进行仿真控制、算法切换、规划参数设置、算法过程演示、起飞点管理、自定义障碍物编辑、导出和批量实验。
